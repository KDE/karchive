    ...
    for (;;) { // repeat until 'end of entries' signature is reached
        ...
        if (!memcmp(buffer, "PK\3\4", 4)) { // local file header
            ...
            */
            ...
            qint64 extraFieldEnd = dev->pos() + extralen;
...
bool KZip::closeArchive()
{
    ...
    for (KZipFileEntry *entry : d->m_fileList) {
        ...
        bool compressedSizeNeeds64Bit = mysize1 >= 0xFFFFFFFF;
        bool uncompressedSizeNeeds64Bit = myusize >= 0xFFFFFFFF;
        ...
        if (!isZip64File) {
            ...
            char z64Buffer[16];
            qToLittleEndian<quint64>(myusize, z64Buffer);
            qToLittleEndian<quint64>(mysize1, z64Buffer + 8);
    ...
    for (KZipFileEntry *entry : d->m_fileList) {
        ...
        if (entryNeedsZip64) {
            ...
            qToLittleEndian<quint32>(myusize, buffer + 24);
        }
        ...
        if (entryNeedsZip64) {
            ...
            // uncompressed file size
            qToLittleEndian<quint64>(myusize, extfield + 4);

            // compressed file size
            qToLittleEndian<quint64>(mysize1, extfield + 12);
